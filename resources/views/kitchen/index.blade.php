    ...
    <div class="min-h-screen bg-[#0b1329] text-slate-100 py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <!-- Main Kitchen Orders Area (Left Side - 2 Columns) -->
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex justify-between items-start mb-3">
                                            ...
                                            <span class="px-3 py-1 text-[10px] font-black uppercase rounded-full tracking-wider border 
                                                ...
                                                {{ in_array($order->status, ['preparing', 'processing']) ? 'bg-blue-500/10 text-blue-400 border-blue-500/30 animate-pulse' : '' }}">
                                                {{ $order->status }}
                                            </span>
                                        </div>
                            ...
                            @empty
                                <div class="col-span-2 bg-slate-900/50 border border-slate-800/80 rounded-3xl p-8 text-center">
                    ...
                    <div class="space-y-5 pt-4 border-t border-slate-800/80">
                        <div class="flex items-center justify-between">
                            <h2 class="text-lg font-bold text-white flex items-center gap-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @forelse ($readyOrders ?? [] as $readyOrder)
                                <div class="bg-slate-900/90 border border-emerald-500/30 rounded-3xl p-5 shadow-xl flex flex-col justify-between space-y-4">
                                    <div>
                                        <div class="flex justify-between items-start mb-3">
                                            <div>
                                                <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Order ID</span>
                                                <h3 class="text-xl font-black text-white">#{{ $readyOrder->id }}</h3>
                                                <p class="text-xs text-slate-400 font-medium mt-0.5">
                                                    {{ $readyOrder->user->name ?? 'Guest User' }}
                                                </p>
                                            </div>
                                            <span class="px-3 py-1 text-[10px] font-black uppercase rounded-full tracking-wider border bg-emerald-500/10 text-emerald-400 border-emerald-500/30">
                                                READY
                                            </span>
                                        </div>
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($readyOrder->items as $item)
                                                <div class="flex justify-between text-xs text-slate-300 bg-[#141a26]/60 p-2.5 rounded-xl border border-slate-800/50">
                                                    <span><strong class="text-emerald-400">{{ $item->quantity }}x</strong> {{ $item->item_name }}</span>
                                                    <span class="text-slate-400 font-medium">Rs. {{ number_format($item->unit_price, 2) }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                            ...
                            @empty
                                <div class="col-span-2 bg-slate-900/30 border border-slate-800/60 rounded-3xl p-6 text-center">
                                    <p class="text-xs text-slate-500">No completed ready orders available at the moment.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2 mt-0.5">
                                                ...
                                                @if (isset($ingredient->quantity))
                                                    <span class="text-[10px] text-slate-500 font-bold">• Qty: {{ $ingredient->quantity }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
