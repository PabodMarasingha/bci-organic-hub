<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            @if (!empty($cart))
                ...
                <div class="flex items-center justify-between px-2">
                    ...
                    <span class="text-xs text-slate-500 font-semibold" id="selected-count">0 items selected</span>
                </div>
            @endif

            <!-- Main Checkout Form Wrapping Selected Items -->
            <form id="cart-checkout-form" method="POST" action="{{ route('orders.create') }}">
                ...
                <div class="space-y-4">
                    @forelse ($cart as $index => $item)
                        @php
                            ...
                            $qty = $item['quantity'] ?? 1;
                        @endphp
                        <div class="cart-item group relative bg-slate-900/80 backdrop-blur-xl border border-slate-800/90 hover:border-emerald-500/40 rounded-3xl p-5 sm:p-6 transition-all duration-300 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-5 hover:-translate-y-0.5"
                            ...
                            <div class="flex items-start gap-4 flex-1">
                                <input type="checkbox" name="selected_items[]" value="{{ $index }}" checked onchange="recalculateTotal()"
                                       class="item-checkbox mt-1.5 w-5 h-5 rounded border-slate-700 bg-slate-950 text-emerald-500 focus:ring-emerald-500/30 focus:ring-offset-0 cursor-pointer shrink-0">
                                ...
                                <div class="space-y-2 flex-1">
                                    ...
                                    <div class="flex flex-wrap items-center gap-4 pt-2">
                                        <p class="text-xs text-slate-400">
                                            Unit Price: <span class="text-slate-200 font-semibold">LKR {{ number_format($unitPrice, 2) }}</span>
                                        </p>

                                        <!-- Quantity Control Bar -->
                                        <div class="flex items-center gap-1 bg-slate-950/80 border border-slate-800 rounded-xl px-2 py-1">
                                            <span class="text-[10px] font-bold text-slate-500 uppercase mr-1">Qty:</span>
                                            <button type="button" onclick="adjustQty('{{ $index }}', -1)" class="w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-bold transition">-</button>
                                            <span id="qty-val-{{ $index }}" class="item-qty w-8 text-center text-xs font-black text-emerald-400">{{ $qty }}</span>
                                            <button type="button" onclick="adjustQty('{{ $index }}', 1)" class="w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-bold transition">+</button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center justify-between sm:justify-end gap-6 pt-4 sm:pt-0 border-t sm:border-t-0 border-slate-800/80">
                                <div class="text-left sm:text-right">
                                    <span class="text-[10px] text-slate-500 uppercase tracking-widest block font-bold">Subtotal</span>
                                    <span class="item-subtotal text-xl font-black text-emerald-400" id="subtotal-{{ $index }}">
                                        LKR {{ number_format($unitPrice * $qty, 2) }}
                                    </span>
                                </div>

                                <div class="flex items-center gap-2">
                                    <!-- Order Only This Item -->
                                    <button type="button" onclick="orderSingleItem('{{ $index }}')" 
                                            class="px-3 py-2 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-slate-950 border border-emerald-500/30 rounded-xl text-xs font-bold transition duration-200" 
                                            title="Order this item only">
                                        Order
                                    </button>

                                    <!-- Remove Button -->
                                    <button type="button" onclick="removeCartItem('{{ $index }}')" 
                                            class="p-2 text-slate-500 hover:text-red-400 bg-slate-800/50 hover:bg-red-500/10 border border-slate-800 hover:border-red-500/30 rounded-xl transition duration-200" 
                                            title="Remove item">
                ...
                @if (!empty($cart))
                    <div class="mt-8 bg-slate-900/90 backdrop-blur-2xl border border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
                        <div>
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block">Total Selected Amount</span>
                            <div id="selected-total" class="text-3xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-200 mt-1">
                                LKR 0.00
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 w-full sm:w-auto">
                            <button type="submit" id="checkout-btn"
                               class="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-400 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-black text-sm rounded-2xl shadow-xl shadow-emerald-500/20 hover:shadow-emerald-500/40 transform active:scale-95 transition-all duration-200 text-center flex items-center justify-center gap-2">
                                <span>Proceed Selected to Checkout</span>
    ...
    <!-- Hidden Form for Backend Quantity Synchronization -->
    <form id="update-qty-form" method="POST" action="" class="hidden">
        @csrf
        @method('PATCH')
        <input type="hidden" name="quantity" id="update-qty-input">
    </form>
    ...
    <script>
        ...
        // Recalculate total for checked items
        ...
        // Adjust Item Quantity dynamically
        function adjustQty(index, change) {
            ...
            // Sync with backend session asynchronously
            syncQtyWithBackend(index, newQty);
        }

        // Send backend PATCH request for quantity change
        function syncQtyWithBackend(index, qty) {
            const form = document.getElementById('update-qty-form');
            form.action = `/cart/update/${index}`; // your cart update route pattern
            document.getElementById('update-qty-input').value = qty;
            ...
            fetch(form.action, {
                ...
                body: formData
            }).catch(err => console.error("Quantity sync error:", err));
        }

        // Order single item click
        ...
        // Remove single item trigger
        function removeCartItem(index) {
            if (confirm('Are you sure you want to remove this item?')) {
                const form = document.getElementById('delete-item-form');
                form.action = `/cart/remove/${index}`; // your cart remove route pattern
                form.submit();
            }
        }
    </script>
