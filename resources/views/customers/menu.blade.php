<x-app-layout>
    <div class="min-h-screen bg-[#0b1329] text-slate-100 py-10 px-4 sm:px-6 lg:px-8 selection:bg-cyan-500 selection:text-slate-950">
        <div class="max-w-7xl mx-auto space-y-10">

            <!-- Hero & Header Banner -->
            <div class="relative overflow-hidden bg-gradient-to-r from-slate-900 via-[#131d38] to-slate-900 border border-slate-800/80 p-8 sm:p-10 rounded-3xl shadow-2xl backdrop-blur-xl">
                ...
                <div class="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                    <div class="space-y-3 max-w-2xl">
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-bold uppercase tracking-widest animate-pulse">
                    ...
                    <a href="{{ route('cart') }}" class="group relative inline-flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-black text-sm rounded-2xl shadow-xl shadow-cyan-500/20 transform hover:-translate-y-0.5 transition duration-300 shrink-0">
                        <div class="relative">
                            <svg class="w-6 h-6 text-slate-950 transition group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>
                            @if(session('cart') && count(session('cart')) > 0)
                                <span class="absolute -top-2 -right-2 w-5 h-5 bg-emerald-400 text-slate-950 text-[10px] font-black rounded-full flex items-center justify-center border-2 border-slate-950 animate-bounce">
                                    {{ count(session('cart')) }}
                                </span>
                            @endif
                        </div>
            ...
            @if (session('message'))
                <div class="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 p-4 rounded-2xl shadow-lg backdrop-blur-md animate-fade-in">
            ...
            <div class="flex items-center justify-between border-b border-slate-800/80 pb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Select an item to add directly or build your custom bowl</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div class="group relative bg-slate-900/80 border border-slate-800 hover:border-cyan-500/40 rounded-3xl p-6 shadow-xl hover:shadow-cyan-500/10 transition-all duration-300 flex flex-col justify-between hover:-translate-y-1.5">
                        ...
                        <div class="pt-6 mt-6 border-t border-slate-800/80 space-y-3">
                            ...
                            <form method="POST" action="{{ route('cart.add') }}">
                                ...
                                <button type="submit" class="w-full py-3 bg-slate-800 hover:bg-cyan-500 hover:text-slate-950 text-slate-200 font-bold text-xs rounded-xl border border-slate-700 hover:border-cyan-400 transition duration-300 flex items-center justify-center gap-2 group/btn shadow-md">
                            ...
                            <!-- Custom Build Button -->
                            <a href="{{ route('customer.build', $product->id) }}" class="w-full py-3 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 hover:from-emerald-500 hover:to-teal-500 text-emerald-400 hover:text-slate-950 font-black text-xs rounded-xl border border-emerald-500/30 hover:border-emerald-400 transition duration-300 flex items-center justify-center gap-2 group/custom shadow-md">
                ...
                @empty
                    <div class="col-span-full bg-slate-900/50 border border-slate-800 rounded-3xl p-12 text-center">
                        <div class="w-16 h-16 mx-auto rounded-full bg-slate-800/80 flex items-center justify-center mb-4 text-slate-500">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                            </svg>
                        </div>
                        <h3 class="text-lg font-bold text-slate-300">No Meals Available</h3>
                        <p class="text-xs text-slate-500 mt-1">Our chef is preparing a new healthy menu. Please check back shortly!</p>
                    </div>
                @endforelse
            </div>
